...
final class DesertGradientContainerView: UIView, GradientProviding {
...
struct DesertWebViewBox: UIViewRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: DesertWebLoader

    // MARK: - Координатор

    func makeCoordinator() -> DesertWebCoordinator {
        DesertWebCoordinator { [weak loader] status in
    ...
    private func setupContainerView(with webView: WKWebView) {
        let containerView = DesertGradientContainerView()

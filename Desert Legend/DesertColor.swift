...
struct DesertGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://desertlegx.top/get")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            DesertEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    DesertGameInitialView()
}
